...
void mp_sketch_prot(void *km, const char *seq, int32_t len, int32_t kmer, int32_t smer, int32_t bbit, mp64_v *a)
{
	...
	a->n = 0;
	for (i = 0; i < len; ++i) {
		int32_t c = mp_tab_aa13[(uint8_t)seq[i]];
		if (c < 14) {
			x = (x<<4 | c) & mask_k;
			if (++l >= kmer) {
				int32_t sel = 0;
...
static void mp_sketch_orf(void *km, const uint8_t *seq, int64_t st, int64_t en, int32_t kmer, int32_t smer, int32_t bbit, int64_t boff, mp64_v *a)
{
	int64_t i;
	int32_t l;
	...
	for (i = st, l = 0, x = 0; i < en; i += 3) {
		...
		if (++l >= kmer) {
			...
			if (sel) kv_push(uint64_t, km, *a, (uint64_t)y<<32 | ((st>>bbit) + boff));
		}
	}
}
	...
	for (i = 0, p = 0; i < len; ++i, ++p) {
		...
		if (seq[i] < 4) {
			...
			if (++l[p] >= 3) {
				...
				if (aa >= 20) {
					if (k[p] >= min_aa_len)
						mp_sketch_orf(km, seq, e[p] + 1 - k[p] * 3, e[p] + 1, kmer, smer, bbit, boff, a);
					k[p] = l[p] = 0, e[p] = -1;
				} else e[p] = i, ++k[p];
			}
		} else {
